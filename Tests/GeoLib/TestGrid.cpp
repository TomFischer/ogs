...
TEST(GeoLib, SearchNearestPointInGrid)
{
	...
	GeoLib::Point* res = std::get<0>(grid->getNearestPoint(p0));
...
TEST(GeoLib, SearchNearestPointsInDenseGrid)
{
	...
	GeoLib::Point* res = std::get<0>(grid->getNearestPoint(search_pnt));
	...
	search_pnt[0] = 0;
	res = std::get<0>(grid->getNearestPoint(search_pnt));
	...
	for (std::size_t i(0); i < i_max; i++) {
		...
		for (std::size_t j(0); j < j_max; j++) {
			std::size_t offset1(j * k_max + offset0);
			for (std::size_t k(0); k < k_max; k++) {
				res = std::get<0>(grid->getNearestPoint(*pnts[offset1+k]));
